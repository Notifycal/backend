module "get_user_calendars_lambda" {
  source  = "terraform-aws-modules/lambda/aws"
  version = "~> 7.17"

  function_name          = "get-user-calendars-${var.environment}"
  publish                = local.lambdas_publish
  create_package         = local.lambdas_create_package
  local_existing_package = "${path.root}/../dist/lambdas/api/get-user-calendars.zip"
...
module "get_user_calendars_lambda_alias" {
  source  = "terraform-aws-modules/lambda/aws//modules/alias"
  version = "~> 7.17"

  function_name    = module.get_user_calendars_lambda.lambda_function_name
  function_version = module.get_user_calendars_lambda.lambda_function_version
