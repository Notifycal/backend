data "aws_iam_policy_document" "patch_user_profile_iam_policydoc" {
  statement {
    effect = "Allow"
...
module "patch_user_profile_lambda" {
  ...
  local_existing_package = "${path.root}/../dist/lambdas/api/patch-user-profile.zip"
  ...
  policy_json        = data.aws_iam_policy_document.patch_user_profile_iam_policydoc.json
